...
namespace big
{
	void view::outfit_editor()
	{
		...
		if (ImGui::Begin("Outfit Editor", nullptr, ImGuiWindowFlags_NoTitleBar))
		{
			...
			components::sub_title("Click + drag to move window, click/drag on a side to shrink/expand size.");
			...
			ImGui::SameLine();
			components::command_checkbox<"previewcam">();
			...
			if (ImGui::BeginTabBar("OutfitTabs"))
			{
				if (ImGui::BeginTabItem("Clothing"))
				{
					...
					for (auto& item : components.items)
					{
						...
						{
							outfit::check_bounds_drawable(&item, 0); // The game does this on its own but seems to crash if we call OOB values too fast.
					...
					for (auto& item : components.items)
					{
						ImGui::SetNextItemWidth(120);
						if (ImGui::InputInt(
						        std::format("{} {} [0,{}]##2", item.label, "OUTFIT_TEX"_T, item.texture_id_max).c_str(),
						        &item.texture_id))
						{
							outfit::check_bounds_texture(&item, 0); // The game does this on its own but seems to crash if we call OOB values too fast.
				...
				if (ImGui::BeginTabItem("Accessories"))
				{
					...
					for (auto& item : props.items)
					{
						...
						{
							outfit::check_bounds_drawable(&item, -1); // The game does this on its own but seems to crash if we call OOB values too fast.
					...
					for (auto& item : props.items)
					{
						ImGui::SetNextItemWidth(120);
						if (ImGui::InputInt(
						        std::format("{} {} [0,{}]##4", item.label, "OUTFIT_TEX"_T, item.texture_id_max).c_str(),
						        &item.texture_id))
						{
							outfit::check_bounds_texture(&item, -1); // The game does this on its own but seems to crash if we call OOB values too fast.
			...
			components::button("OUTFIT_DELETE_SELECTED"_T, [saved_outfits] {
				if (selected_index >= 0 && selected_index < saved_outfits.size())
				{
					...
					if (selected_index >= saved_outfits.size())
					{
						selected_index = saved_outfits.size() - 1;
					}
				}
			});
			...
			if (ImGui::BeginListBox("Saved Outfits"))
			{
				for (size_t i = 0; i < saved_outfits.size(); i++)
				{
					...
					if (ImGui::Selectable(saved_outfits[i].c_str(), selected))
					{
						selected_index = i;
					}
				}
